...
export default createFastifyRpcPlugin(productsSchema, {
  ...
  create: async ({ body, request }) => {
    ...
    logActivity(request.server.db, { userId: request.requestContext.get('userId'), action: 'product.created', resourceType: 'product', resourceId: product.id, metadata: { name: product.name, price: product.price } })
  ...
  update: async ({ params, body, request }) => {
    ...
    logActivity(request.server.db, { userId: request.requestContext.get('userId'), action: 'product.updated', resourceType: 'product', resourceId: params.id, metadata: { changes: body } })
    return { status: 200 as const, body: { success: true as const, data: product } }
  },

  delete: async ({ params, request }) => {
    const product = await productService.deleteProduct(request.server.db, params.id)
    logActivity(request.server.db, { userId: request.requestContext.get('userId'), action: 'product.deleted', resourceType: 'product', resourceId: params.id, metadata: { name: product.name, price: product.price } })
